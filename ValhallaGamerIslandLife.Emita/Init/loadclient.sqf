//Request & Load Stats
//Author: {ZAG}Ed!
server globalChat format["Persistent data awaiting load. Please wait...", loadRewardBonus];
hint "Loading Data. Please wait.";
...
if(_loadProceed) then {
	...
	{
	_escpersentage = "%";
	hintsilent format["Loading Data: %1%2",floor ((_ii / count(loadcontent)) * 100),_escpersentage];
	sleep 0.1;
...
server globalChat format["Account Loaded. Your Donator bonus is %1 which will be added to each salary paycheck.", loadRewardBonus];
